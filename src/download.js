...
function loadasset() {
	if (document.getElementById('asset') != undefined) {
		var tmp = document.getElementById('asset').value.split(';;');
		console.log(tmp);
		var i = 0;
		var j = 0;
		while (tmp[i] != "" && tmp[i] != undefined) {
			...
			switch (tmp[i]) {
				case "box":
					box["element" + j] = { nom: tmp[i + 1], url: tmp[i + 2] };
					break;
				case "background":
					back["element" + j] = { nom: tmp[i + 1], url: tmp[i + 2] };
					break;
				case "skill":
					predate["element" + j] = { nom: tmp[i + 1], url: tmp[i + 2] };
					break;
				case "portrait":
					portrait["element" + j] = { nom: tmp[i + 1], url: tmp[i + 2] };
					break;
				case "misc":
					misc["element" + j] = { nom: tmp[i + 1], url: tmp[i + 2] };
					break;
			}
			i += 3;
			j++;
		}
		document.getElementById('asset').parentNode.removeChild(document.getElementById('asset'));
		console.log("asset=" + box);
	}
}
...
function SendCanvas(option) {
	...
	console.log(data);
	if (r != 0) {
		data.objects[r - 1].selectable = false;
		while (r - 2 > -1) {
			delete data.objects[r - 2];
			r--;
		}
	}
	...
	if (canvas._objects.length == 0) {
		...
		if (option == 1) {
			...
			var name = json["element" + nbrsheet].nom;
		...
		xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
		xmlhttp.send(encodeURI('username=' + user + '&tableName=' + name + '&data=' + data));
	}
}
