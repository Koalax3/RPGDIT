...
function Save_Asset($username)
{
    ...
    $reponse = $bdd->query("SELECT * FROM asset WHERE 1 ORDER BY  `nom` ASC");
    while ($donnees = $reponse->fetch())
    {
        $asset .= $donnees['type'].';;'.$donnees['nom'].';;'.$donnees['image'].';;';
    }
    return $asset;
}
...
if ($_POST['recupe'])
{
    ...
    $asset = Save_asset($username);
    echo "<option id='asset' style='display:none;' value='".$asset."'></option>";    
